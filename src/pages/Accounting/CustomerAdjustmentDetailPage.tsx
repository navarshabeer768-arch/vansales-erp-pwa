...
export function CustomerAdjustmentDetailPage() {
  ...
  const { overallStatus: approvalOverallStatus, steps: approvalSteps, submitForApproval, processAction } = useAdjustmentApprovals('customer_adjustments', id);
  const { posting, post, retry } = useAdjustmentPosting('customer_adjustments');
  const { history: postingHistory } = useAdjustmentPostingHistory('customer_adjustments', id);
  const { request: reversalRequest, createReversalRequest } = useAdjustmentReversal('customer_adjustments', id);
  const { push } = useToast();
  ...
  const handlePost = async () => {
    if (!confirm('Post this adjustment? This will adjust the customer ledger and cannot be undone through editing.')) return;
    const { data, error } = await post(doc.id);
    if (error) { push('error', error); reload(); return; }
    push('success', `Adjustment posted — ${(data as any)?.invoice_credited ?? 0} credited, ${(data as any)?.unallocated ?? 0} unallocated.`);
    reload();
  };
  ...
  const handleRequestReversal = async () => {
    const reason = prompt('Reason for requesting a reversal of this posted adjustment:');
    if (!reason) return;
    ...
    <div className="space-y-6">
      ...
      {tab === 'reversal' && (
        <div className="card p-4 text-sm">
          ...
          ) : <p className="text-slate-500">No reversal has been requested for this adjustment.</p>}
        </div>
      )}
