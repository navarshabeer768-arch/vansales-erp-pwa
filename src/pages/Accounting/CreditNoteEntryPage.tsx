...
import { useCreateCreditNote, CreditNoteItemInput } from '@/hooks/useCreditNotes';
import { useToast } from '@/contexts/ToastContext';

export function CreditNoteEntryPage() {
  const navigate = useNavigate();
  const { push } = useToast();
  const { customers } = useCustomers();
  const { documentTypesFor, reasonsFor } = useFinancialAdjustmentCatalogs();
  const { submit, submitting } = useCreateCreditNote();

  const [customerId, setCustomerId] = useState('');
  const [documentTypeCode, setDocumentTypeCode] = useState('manual_credit_note');
  ...
  const docTypes = documentTypesFor('credit_note');
  const reasons = reasonsFor('credit_note');
  ...
  const handleSave = async (submitAfter: boolean) => {
    ...
    if (selectedDocType?.invoice_required && !invoiceId) { push('error', 'This credit note type requires an invoice.'); return; }

    let items: CreditNoteItemInput[] = [];
    let amountOnlyValue: number | undefined;

    if (entryMode === 'amount') {
      if (!amountValue || Number(amountValue) <= 0) { push('error', 'Enter a credit amount.'); return; }
    ...
    if (submitAfter && data) {
      await supabase.rpc('change_credit_note_status', { p_id: data, p_new_status: 'submitted' });
    }
    push('success', submitAfter ? 'Credit note submitted.' : 'Credit note saved as draft.');
    if (data) navigate(`/accounting/credit-notes/${data}`);
  };

  return (
    <div className="mx-auto max-w-2xl space-y-4 pb-8">
      <h1 className="flex items-center gap-2 text-lg font-bold text-slate-800 dark:text-slate-100">
        <FileMinus size={20} /> New Credit Note
      </h1>
      ...
      <div className="card grid grid-cols-1 gap-3 p-4 sm:grid-cols-2">
        ...
        <div>
          <label className="label">Credit Note Type</label>
          ...
          <div className="space-y-2">
            {invoiceItems.map((item) => (
              <div key={item.invoice_item_id} className="rounded-lg border border-slate-100 p-3 dark:border-slate-800">
                ...
                <input
                  type="number" inputMode="decimal" className="input mt-2 !h-[40px]" placeholder="Credit amount"
