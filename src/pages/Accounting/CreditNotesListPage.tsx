...
export function CreditNotesListPage() {
  ...
  const { push } = useToast();
  const { notes, loading, submitDraft, cancelDraft } = useCreditNotes({ dateFrom, dateTo, status: status || undefined });
  ...
  const columns: Column<CreditNoteRow>[] = [
    {
      ...
      render: (r) => (
        <button className="font-medium text-blue-600 hover:underline dark:text-blue-400" onClick={() => navigate(`/accounting/credit-notes/${r.id}`)}>
          {r.document_number}
        </button>
      ),
    },
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="flex items-center gap-2 text-xl font-bold text-slate-800 dark:text-slate-100">
            <FileMinus size={20} /> Credit Notes
          </h1>
          <p className="text-sm text-slate-500">Draft credit notes — nothing here has posted or adjusted a customer balance yet.</p>
        </div>
        <PermissionGate permission="financial_adjustments:create_credit_note">
          <button className="btn-primary" onClick={() => navigate('/accounting/credit-notes/new')}>
