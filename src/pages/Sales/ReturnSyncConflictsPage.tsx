...
interface OpenReturnConflict {
  id: string;
  return_id: string;
  conflict_type: string;
  conflict_details: Record<string, unknown>;
  detected_at: string;
  return?: { return_number: string; customer?: { business_name: string } | null } | null;
}
...
function useOpenReturnSyncConflicts() {
  const { company } = useAuth();
  const [conflicts, setConflicts] = useState<OpenReturnConflict[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    const { data } = await supabase
      .from('sales_return_sync_conflicts')
      .select('*, return:sales_returns(return_number, customer:customers(business_name))')
      .eq('company_id', company.id)
      .eq('status', 'open')
      .order('detected_at');
    setConflicts((data ?? []) as unknown as OpenReturnConflict[]);
    setLoading(false);
  }, [company]);
  ...
  const resolve = useCallback(async (conflictId: string, resolution: string, notes?: string) => {
    const { error } = await supabase.rpc('resolve_return_sync_conflict', { p_conflict_id: conflictId, p_resolution: resolution, p_notes: notes ?? null });
...
export function ReturnSyncConflictsPage() {
  const { conflicts, loading, resolve } = useOpenReturnSyncConflicts();
  const { push } = useToast();
    ...
    <div className="space-y-6">
      <div>
        <h1 className="flex items-center gap-2 text-xl font-bold text-slate-800 dark:text-slate-100">
          <AlertTriangle size={20} /> Return Sync Conflicts
        </h1>
        <p className="text-sm text-slate-500">
          Offline returns that came back online with data that no longer matches the server — invoice validity,
          returnable quantity, or customer status changed since the return was created offline.
        </p>
      </div>
        ...
        {conflicts.map((c) => (
          <div key={c.id} className="card p-4">
            <div className="flex items-start justify-between">
              <div>
                <button className="font-medium text-blue-600 hover:underline dark:text-blue-400" onClick={() => navigate(`/sales/returns/${c.return_id}`)}>
                  {c.return?.return_number ?? c.return_id}
                </button>
                <p className="text-sm text-slate-500">{c.return?.customer?.business_name}</p>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              <PermissionGate permission="sales_returns:resolve_sync_conflict">
