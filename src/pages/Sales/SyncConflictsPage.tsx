...
interface OpenConflict {
  ...
  detected_at: string;
  order?: { order_number: string; customer?: { business_name: string } | null } | null;
}

const RESOLUTION_OPTIONS = [
  { value: 'use_server_values', label: 'Use Server Values' },
  { value: 'keep_local_pending_approval', label: 'Keep Local (Pending Approval)' },
  { value: 'merge_non_conflicting', label: 'Merge Non-Conflicting Changes' },
...
function useOpenSyncConflicts() {
  ...
  const load = useCallback(async () => {
    ...
    const { data } = await supabase
      .from('sales_order_sync_conflicts')
      .select('*, order:sales_orders(order_number, customer:customers(business_name))')
      .eq('company_id', company.id)
      .eq('status', 'open')
      .order('detected_at');
    setConflicts((data ?? []) as unknown as OpenConflict[]);
    setLoading(false);
  }, [company]);
...
export function SyncConflictsPage() {
  const { conflicts, loading, resolve } = useOpenSyncConflicts();
  const { push } = useToast();
    ...
    <div className="space-y-6">
      <div>
        <h1 className="flex items-center gap-2 text-xl font-bold text-slate-800 dark:text-slate-100">
          <AlertTriangle size={20} /> Sync Conflicts
        </h1>
        <p className="text-sm text-slate-500">
          Orders that came back online with data that no longer matches the server — customer/product status,
          pricing, stock, or credit changed since the order was created offline. Nothing here was auto-approved.
        </p>
      </div>
        ...
        {conflicts.map((c) => (
          <div key={c.id} className="card p-4">
            <div className="flex items-start justify-between">
              <div>
                <button className="font-medium text-blue-600 hover:underline dark:text-blue-400" onClick={() => navigate(`/sales/orders/${c.order_id}`)}>
                  {c.order?.order_number ?? c.order_id}
                </button>
                <p className="text-sm text-slate-500">{c.order?.customer?.business_name}</p>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              <PermissionGate permission="sales_orders:resolve_sync_conflict">
