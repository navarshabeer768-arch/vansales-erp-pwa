-- ============================================================================
-- 0117_adjustment_dashboard_notifications.sql
-- Continues 0113-0116. Extends dashboard_stats() with real Part 2
-- approval/posting/allocation/reversal KPIs — full prior widget set
-- preserved and appended to (checked against 0112's actual output).
-- ============================================================================

create or replace function notify_adjustment_lifecycle_event(p_document_table text, p_document_id uuid, p_user_id uuid, p_type text, p_title text, p_message text)
...
$$;
grant execute on function notify_adjustment_lifecycle_event(text, uuid, uuid, text, text, text) to authenticated;

create or replace function submit_adjustment_for_approval_notified(p_document_table text, p_document_id uuid)
returns uuid language plpgsql security definer as $$
declare v_approval_id uuid; v_created_by uuid; v_status text;
begin
  v_approval_id := submit_adjustment_for_approval(p_document_table, p_document_id);
  execute format('select created_by, approval_status from %I where id = $1', p_document_table) into v_created_by, v_status using p_document_id;
  if v_status = 'pending' then
    perform notify_adjustment_lifecycle_event(p_document_table, p_document_id, v_created_by, 'approval_notification', 'Pending Approval', 'A document is pending your approval.');
  end if;
  return v_approval_id;
end;
$$;
grant execute on function submit_adjustment_for_approval_notified(text, uuid) to authenticated;

create or replace function process_adjustment_approval_action_notified(p_step_id uuid, p_action text, p_reason text default null, p_notes text default null)
returns void language plpgsql security definer as $$
declare v_document_table text; v_document_id uuid; v_created_by uuid;
begin
  select a.document_table, a.document_id into v_document_table, v_document_id
  from financial_adjustment_approval_steps s join financial_adjustment_approvals a on a.id = s.approval_id where s.id = p_step_id;

  perform process_adjustment_approval_action(p_step_id, p_action, p_reason, p_notes);
  execute format('select created_by from %I where id = $1', v_document_table) into v_created_by using v_document_id;

  if p_action = 'approve' then
    perform notify_adjustment_lifecycle_event(v_document_table, v_document_id, v_created_by, 'approval_notification', 'Approved', 'Your document was approved.');
  elsif p_action = 'reject' then
    perform notify_adjustment_lifecycle_event(v_document_table, v_document_id, v_created_by, 'system', 'Rejected', format('Rejected: %s', coalesce(p_reason, '')));
  elsif p_action = 'return_for_correction' then
    perform notify_adjustment_lifecycle_event(v_document_table, v_document_id, v_created_by, 'system', 'Returned for Correction', format('Returned: %s', coalesce(p_reason, '')));
  end if;
end;
$$;
grant execute on function process_adjustment_approval_action_notified(uuid, text, text, text) to authenticated;

create or replace function post_credit_note_notified(p_id uuid, p_device_uid text default null, p_is_offline boolean default false)
returns jsonb language plpgsql security definer as $$
declare v_result jsonb; v_created_by uuid; v_number text;
begin
  select created_by, document_number into v_created_by, v_number from credit_notes where id = p_id;
  begin
    v_result := post_credit_note(p_id, p_device_uid, p_is_offline);
    perform notify_adjustment_lifecycle_event('credit_notes', p_id, v_created_by, 'system', 'Posted', format('Credit note %s was posted.', v_number));
    return v_result;
  exception when others then
    perform notify_adjustment_lifecycle_event('credit_notes', p_id, v_created_by, 'system', 'Posting Failed', format('Credit note %s failed to post: %s', v_number, sqlerrm));
    raise;
  end;
end;
$$;
grant execute on function post_credit_note_notified(uuid, text, boolean) to authenticated;

create or replace function post_debit_note_notified(p_id uuid, p_device_uid text default null, p_is_offline boolean default false)
returns jsonb language plpgsql security definer as $$
declare v_result jsonb; v_created_by uuid; v_number text;
begin
  select created_by, document_number into v_created_by, v_number from debit_notes where id = p_id;
  begin
    v_result := post_debit_note(p_id, p_device_uid, p_is_offline);
    perform notify_adjustment_lifecycle_event('debit_notes', p_id, v_created_by, 'system', 'Posted', format('Debit note %s was posted.', v_number));
    return v_result;
  exception when others then
    perform notify_adjustment_lifecycle_event('debit_notes', p_id, v_created_by, 'system', 'Posting Failed', format('Debit note %s failed to post: %s', v_number, sqlerrm));
    raise;
  end;
end;
$$;
grant execute on function post_debit_note_notified(uuid, text, boolean) to authenticated;

create or replace function post_customer_adjustment_notified(p_id uuid, p_device_uid text default null, p_is_offline boolean default false)
returns jsonb language plpgsql security definer as $$
declare v_result jsonb; v_created_by uuid; v_number text;
begin
  select created_by, document_number into v_created_by, v_number from customer_adjustments where id = p_id;
  begin
    v_result := post_customer_adjustment(p_id, p_device_uid, p_is_offline);
    perform notify_adjustment_lifecycle_event('customer_adjustments', p_id, v_created_by, 'system', 'Posted', format('Adjustment %s was posted.', v_number));
    return v_result;
  exception when others then
    perform notify_adjustment_lifecycle_event('customer_adjustments', p_id, v_created_by, 'system', 'Posting Failed', format('Adjustment %s failed to post: %s', v_number, sqlerrm));
    raise;
  end;
end;
$$;
grant execute on function post_customer_adjustment_notified(uuid, text, boolean) to authenticated;

create or replace function create_adjustment_reversal_request_notified(p_document_table text, p_document_id uuid, p_reason text)
returns uuid language plpgsql security definer as $$
declare v_request_id uuid; v_created_by uuid;
begin
  v_request_id := create_adjustment_reversal_request(p_document_table, p_document_id, p_reason);
  execute format('select created_by from %I where id = $1', p_document_table) into v_created_by using p_document_id;
  perform notify_adjustment_lifecycle_event(p_document_table, p_document_id, v_created_by, 'system', 'Reversal Requested', 'A reversal was requested for this document.');
  return v_request_id;
end;
$$;
grant execute on function create_adjustment_reversal_request_notified(text, uuid, text) to authenticated;
...
begin
  ...
  select jsonb_build_object(
    ...
    -- Phase 5B.4 Part 2 widgets — real posted-document data.
    'adjustment_pending_approvals', (
      (select count(*) from credit_notes where company_id = v_company_id and status = 'pending_approval') +
      (select count(*) from debit_notes where company_id = v_company_id and status = 'pending_approval') +
      (select count(*) from customer_adjustments where company_id = v_company_id and status = 'pending_approval')
    ),
    'adjustment_approved_today', (
      (select count(*) from financial_adjustment_approvals where company_id = v_company_id and overall_status = 'approved' and updated_at::date = v_today)
    ),
    'adjustment_posted_today', (
      (select count(*) from credit_notes where company_id = v_company_id and posting_status = 'posted' and posted_date::date = v_today) +
      (select count(*) from debit_notes where company_id = v_company_id and posting_status = 'posted' and posted_date::date = v_today) +
      (select count(*) from customer_adjustments where company_id = v_company_id and posting_status = 'posted' and posted_date::date = v_today)
    ),
    'credit_note_amount_posted_today', coalesce((select sum(net_amount) from credit_notes where company_id = v_company_id and posting_status = 'posted' and posted_date::date = v_today), 0),
    'debit_note_amount_posted_today', coalesce((select sum(net_amount) from debit_notes where company_id = v_company_id and posting_status = 'posted' and posted_date::date = v_today), 0),
    'adjustment_pending_posting', (
      (select count(*) from credit_notes where company_id = v_company_id and status = 'ready_to_post') +
      (select count(*) from debit_notes where company_id = v_company_id and status = 'ready_to_post') +
      (select count(*) from customer_adjustments where company_id = v_company_id and status = 'ready_to_post')
    ),
    'adjustment_failed_posting', (
      (select count(*) from credit_notes where company_id = v_company_id and status = 'posting_failed') +
      (select count(*) from debit_notes where company_id = v_company_id and status = 'posting_failed') +
      (select count(*) from customer_adjustments where company_id = v_company_id and status = 'posting_failed')
    ),
    'adjustment_pending_allocation', (select count(*) from customer_unallocated_credits where company_id = v_company_id and credit_note_id is not null and status in ('available', 'partially_allocated')),
    'adjustment_reversal_requests_pending', (select count(*) from customer_adjustment_reversals where company_id = v_company_id and approval_status = 'pending'),
    'adjustment_offline_conflicts_open', (select count(*) from adjustment_sync_conflicts where company_id = v_company_id and status = 'open')
  ) into v_result;
