-- ============================================================================
-- 0064_invoice_dashboard_widgets.sql
-- Continues 0059-0063. Extends dashboard_stats() with real invoice-draft
-- KPIs — explicitly NOT counted as finalized sales revenue anywhere.
-- ============================================================================
...
begin
  ...
  select jsonb_build_object(
    ...
    'draft_invoices_today', (select count(*) from sales_invoices where company_id = v_company_id and invoice_date = v_today and status != 'cancelled_before_posting'),
    'draft_invoice_value_today', coalesce((select sum(net_amount) from sales_invoices where company_id = v_company_id and invoice_date = v_today and status != 'cancelled_before_posting'), 0),
    ...
    'direct_invoice_drafts', (select count(*) from sales_invoices where company_id = v_company_id and is_direct_invoice and invoice_date = v_today),
    'cash_invoice_drafts', (select count(*) from sales_invoices where company_id = v_company_id and payment_type = 'cash' and invoice_date = v_today),
    'credit_invoice_drafts', (select count(*) from sales_invoices where company_id = v_company_id and payment_type = 'credit' and invoice_date = v_today),
    'hybrid_invoice_drafts', (select count(*) from sales_invoices where company_id = v_company_id and payment_type = 'hybrid' and invoice_date = v_today),
    'offline_invoice_drafts_pending_sync', (select count(*) from sales_invoices where company_id = v_company_id and status = 'sync_pending'),
    'invoice_sync_failed', (select count(*) from sales_invoices where company_id = v_company_id and status = 'sync_failed')
  ) into v_result;
