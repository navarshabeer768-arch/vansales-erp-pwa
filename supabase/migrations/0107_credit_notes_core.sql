...
create table credit_notes (
  ...
  original_return_id uuid references sales_returns(id) on delete set null,
...
create index idx_credit_notes_company_date on credit_notes(company_id, document_date);
...
create index idx_credit_notes_status on credit_notes(company_id, status);

alter table credit_notes enable row level security;
create policy credit_notes_isolation on credit_notes for all
...
create index idx_credit_note_items_invoice_item on credit_note_items(original_invoice_item_id);

alter table credit_note_items enable row level security;
create policy credit_note_items_isolation on credit_note_items for all
  using (company_id = current_company_id()) with check (company_id = current_company_id());
...
create or replace function create_credit_note_draft(
  ...
  p_original_return_id uuid default null,
...
begin
  ...
  select id into v_existing_id from credit_notes where company_id = v_company_id and client_uuid = p_client_uuid;
  if v_existing_id is not null then return v_existing_id; end if;

  select * into v_doc_type from financial_document_types
  where code = p_document_type_code and document_category = 'credit_note' and (company_id is null or company_id = v_company_id) and is_active
  order by company_id nulls last limit 1;
  if not found then raise exception 'Unknown or inactive credit note type: %', p_document_type_code; end if;
  ...
  if v_doc_type.return_required and p_original_return_id is null then
    raise exception '% requires an original sales return', v_doc_type.label;
  end if;

  if p_reason_code is not null then
    select id into v_reason_id from financial_adjustment_reasons
    where code = p_reason_code and applies_to in ('credit_note', 'all') and (company_id is null or company_id = v_company_id)
    order by company_id nulls last limit 1;
  end if;

  v_document_number := next_financial_document_no('credit_note');
  v_initial_status := case when p_is_offline then 'sync_pending' else 'draft' end;

  insert into credit_notes (
    company_id, document_number, document_type_id, customer_id, original_invoice_id, original_return_id,
    ...
    v_company_id, v_document_number, v_doc_type.id, p_customer_id, p_original_invoice_id, p_original_return_id,
  ...
  if p_amount_only_value is not null and jsonb_array_length(p_items) = 0 then
    ...
    for v_item in select * from jsonb_array_elements(p_items) loop
      ...
      if v_line_amount = 0 and (v_item->>'corrected_discount') is not null and (v_item->>'original_discount') is not null then
        v_line_amount := (v_item->>'original_discount')::numeric - (v_item->>'corrected_discount')::numeric;
      end if;
      if v_line_amount = 0 and (v_item->>'corrected_tax') is not null and (v_item->>'original_tax') is not null then
        v_line_amount := (v_item->>'original_tax')::numeric - (v_item->>'corrected_tax')::numeric;
      end if;

      insert into credit_note_items (
        company_id, credit_note_id, original_invoice_item_id, product_id, variant_id, description, uom_id, quantity, unit_price,
        ...
        case when (v_item->>'corrected_discount') is not null then (v_item->>'original_discount')::numeric - (v_item->>'corrected_discount')::numeric end,
        (v_item->>'original_tax')::numeric, (v_item->>'corrected_tax')::numeric,
        case when (v_item->>'corrected_tax') is not null then (v_item->>'original_tax')::numeric - (v_item->>'corrected_tax')::numeric end,
        v_line_amount,
  ...
  update credit_notes set gross_amount = v_gross, discount_amount = v_discount, tax_amount = v_tax, net_amount = v_net where id = v_id;

  insert into adjustment_status_history (company_id, document_table, document_id, old_status, new_status, changed_by)
  values (v_company_id, 'credit_notes', v_id, null, v_initial_status, auth.uid());

  return v_id;
end;
$$;
grant execute on function create_credit_note_draft(
